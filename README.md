...
The `ether_mac_gmii_1g` block is a synchronous GMII MAC wrapper intended for
basic 1 Gbit/s integration and simulation.  FIFO-wrapped stream variants,
standalone async stream FIFOs, and pause-frame helpers are included; MII/RGMII
rate adaptation, 10G PHY/MAC blocks, PTP timestamping, and vendor-specific IO
register wrappers are still outside the implemented scope.
