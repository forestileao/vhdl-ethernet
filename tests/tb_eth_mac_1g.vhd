...
entity tb_eth_mac_1g is
end entity;

architecture sim of tb_eth_mac_1g is
...
begin
    ...
    begin
        ...
        for i in 0 to FRAME_LEN - 1 loop
            tx_data  <= std_logic_vector(to_unsigned((16#40# + i) mod 256, 8));
            tx_valid <= '1';
        ...
        assert done report "GMII MAC loopback timed out" severity failure;
        assert bad_frame = '0' report "GMII MAC reported bad frame" severity failure;
        assert bad_fcs = '0' report "GMII MAC reported bad FCS" severity failure;
        finish;
    end process;
    ...
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                assert rx_data = std_logic_vector(to_unsigned((16#40# + pos) mod 256, 8))
                    report "GMII MAC loopback byte mismatch" severity failure;
                assert rx_user = '0' report "GMII MAC loopback user flag set" severity failure;

                if rx_last = '1' then
                    assert pos = FRAME_LEN - 1 report "GMII MAC loopback ended at wrong length" severity failure;
                    done <= true;
                end if;
                pos := pos + 1;
            end if;
