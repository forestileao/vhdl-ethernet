...
entity axis64_eth_fcs_strip is
    generic (
...
architecture rtl of axis64_eth_fcs_strip is
    type byte_mem_t is array (0 to MAX_FRAME_BYTES - 1) of byte_t;
    type state_t is (COLLECT, EMIT);

    signal state : state_t := COLLECT;
    signal mem : byte_mem_t := (others => (others => '0'));
    signal in_len : natural range 0 to MAX_FRAME_BYTES := 0;
...
begin
    ...
    m_axis_tuser <= frame_user when state = EMIT and out_pos + 8 >= payload_len else '0';
                        ...
                        if s_axis_tvalid = '1' then
                            ...
                            for lane in 0 to 7 loop
                                if s_axis_tkeep(lane) = '1' then
                                    if wr_len < MAX_FRAME_BYTES then
                                        ...
                                        frame(wr_len) := lane_byte(s_axis_tdata, lane);
                                        wr_len := wr_len + 1;
                                    else
                                        stored_error := '1';
                                    end if;
                            ...
                            if s_axis_tlast = '1' then
                                if wr_len < 5 then
                                    ...
                                    if got /= expect or stored_error = '1' or s_axis_tuser = '1' then
                                        frame_user <= '1';
                                        bad_fcs_reg <= '1';
                                    else
                                        frame_user <= '0';
