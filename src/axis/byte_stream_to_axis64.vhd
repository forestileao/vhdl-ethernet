-- SPDX-License-Identifier: MIT

library ieee;
use ieee.std_logic_1164.all;

library work;
use work.eth_types_pkg.all;

entity byte_stream_to_axis64 is
    port (
        clk           : in  std_logic;
        rst           : in  std_logic;
        s_axis_tdata  : in  byte_t;
        s_axis_tvalid : in  std_logic;
        s_axis_tready : out std_logic;
        s_axis_tlast  : in  std_logic;
        s_axis_tuser  : in  std_logic;
        m_axis_tdata  : out word64_t;
        m_axis_tkeep  : out keep8_t;
        m_axis_tvalid : out std_logic;
        m_axis_tready : in  std_logic;
        m_axis_tlast  : out std_logic;
        m_axis_tuser  : out std_logic
    );
end entity;

architecture rtl of byte_stream_to_axis64 is
    signal data_reg : word64_t := (others => '0');
    signal keep_reg : keep8_t := (others => '0');
    signal count : natural range 0 to 7 := 0;
    signal out_valid : std_logic := '0';
    signal out_last : std_logic := '0';
    signal out_user : std_logic := '0';
begin
    s_axis_tready <= '1' when out_valid = '0' else '0';
    m_axis_tdata <= data_reg;
    m_axis_tkeep <= keep_reg;
    m_axis_tvalid <= out_valid;
    m_axis_tlast <= out_last;
    m_axis_tuser <= out_user;

    process (clk)
        variable next_data : word64_t;
        variable next_keep : keep8_t;
    begin
        if rising_edge(clk) then
            if rst = '1' then
                data_reg <= (others => '0');
                keep_reg <= (others => '0');
                count <= 0;
                out_valid <= '0';
                out_last <= '0';
                out_user <= '0';
            else
                if out_valid = '1' and m_axis_tready = '1' then
                    out_valid <= '0';
                    out_last <= '0';
                    out_user <= '0';
                    data_reg <= (others => '0');
                    keep_reg <= (others => '0');
                    count <= 0;
                elsif out_valid = '0' and s_axis_tvalid = '1' then
                    next_data := data_reg;
                    next_keep := keep_reg;
                    next_data(count * 8 + 7 downto count * 8) := s_axis_tdata;
                    next_keep(count) := '1';
                    data_reg <= next_data;
                    keep_reg <= next_keep;

                    if count = 7 or s_axis_tlast = '1' then
                        out_valid <= '1';
                        out_last <= s_axis_tlast;
                        out_user <= s_axis_tuser;
                    else
                        count <= count + 1;
                    end if;
                end if;
            end if;
        end if;
    end process;
end architecture;
