-- SPDX-License-Identifier: MIT

library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

library work;
use work.eth_types_pkg.all;

entity eth_axis_rx is
    port (
        clk                  : in  std_logic;
        rst                  : in  std_logic;
        s_axis_tdata         : in  byte_t;
        s_axis_tvalid        : in  std_logic;
        s_axis_tready        : out std_logic;
        s_axis_tlast         : in  std_logic;
        s_axis_tuser         : in  std_logic;
        m_hdr_valid          : out std_logic;
        m_hdr_ready          : in  std_logic;
        m_eth_dest_mac       : out mac_addr_t;
        m_eth_src_mac        : out mac_addr_t;
        m_eth_type           : out word16_t;
        m_axis_payload_tdata : out byte_t;
        m_axis_payload_tvalid: out std_logic;
        m_axis_payload_tready: in  std_logic;
        m_axis_payload_tlast : out std_logic;
        m_axis_payload_tuser : out std_logic;
        error_header_early   : out std_logic
    );
end entity;

architecture rtl of eth_axis_rx is
    type state_t is (S_HEADER, S_PAYLOAD);
    signal state       : state_t := S_HEADER;
    signal ptr         : natural range 0 to 13 := 0;
    signal dst_reg     : mac_addr_t := (others => '0');
    signal src_reg     : mac_addr_t := (others => '0');
    signal type_reg    : word16_t := (others => '0');
    signal hdr_valid_i : std_logic := '0';
    signal header_err  : std_logic := '0';
begin
    m_hdr_valid           <= hdr_valid_i;
    m_eth_dest_mac        <= dst_reg;
    m_eth_src_mac         <= src_reg;
    m_eth_type            <= type_reg;
    error_header_early    <= header_err;
    s_axis_tready         <= '1' when state = S_HEADER else
                             (m_axis_payload_tready and (m_hdr_ready or not hdr_valid_i));
    m_axis_payload_tvalid <= s_axis_tvalid when state = S_PAYLOAD and (m_hdr_ready = '1' or hdr_valid_i = '0') else '0';
    m_axis_payload_tdata  <= s_axis_tdata;
    m_axis_payload_tlast  <= s_axis_tlast when state = S_PAYLOAD else '0';
    m_axis_payload_tuser  <= s_axis_tuser when state = S_PAYLOAD else '0';

    process (clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
                state       <= S_HEADER;
                ptr         <= 0;
                dst_reg     <= (others => '0');
                src_reg     <= (others => '0');
                type_reg    <= (others => '0');
                hdr_valid_i <= '0';
                header_err  <= '0';
            else
                header_err <= '0';
                if hdr_valid_i = '1' and m_hdr_ready = '1' then
                    hdr_valid_i <= '0';
                end if;

                if s_axis_tvalid = '1' and s_axis_tready = '1' then
                    case state is
                        when S_HEADER =>
                            case ptr is
                                when 0 to 5   => dst_reg(47 - ptr * 8 downto 40 - ptr * 8) <= s_axis_tdata;
                                when 6 to 11  => src_reg(47 - (ptr - 6) * 8 downto 40 - (ptr - 6) * 8) <= s_axis_tdata;
                                when 12       => type_reg(15 downto 8) <= s_axis_tdata;
                                when others   => type_reg(7 downto 0) <= s_axis_tdata;
                            end case;

                            if s_axis_tlast = '1' and ptr /= 13 then
                                header_err <= '1';
                                ptr        <= 0;
                            elsif ptr = 13 then
                                hdr_valid_i <= '1';
                                ptr         <= 0;
                                state       <= S_PAYLOAD;
                                if s_axis_tlast = '1' then
                                    state <= S_HEADER;
                                end if;
                            else
                                ptr <= ptr + 1;
                            end if;

                        when S_PAYLOAD =>
                            if s_axis_tlast = '1' then
                                state <= S_HEADER;
                            end if;
                    end case;
                end if;
            end if;
        end if;
    end process;
end architecture;
